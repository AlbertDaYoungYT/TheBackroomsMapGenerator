...
class CreateBiome:
    def __init__(self, sub, baseScale, scaleToRange, location, gridStep, offset, defaultMaxLength):
        ...
        self.center = floor(baseScale * 0.5)
    ...
    def GenerateLights(self, amount):
        ...
        for x in range(rang):
            ...
            if [x, y, z] in biomeLocations:
                ...
                biomeLocations.append([x, y, z])
                bpy.ops.mesh.primitive_cube_add(location=(x - self.center,y - self.center,z), scale=(0.5, 0.5, 0.1 * 0.5))
                ob = bpy.context.active_object
...
bpy.ops.mesh.primitive_cube_add(location=(0, 0, -0.25), scale=(baseScale * 0.5, baseScale * 0.5, 0.5 * 0.5))
ob = bpy.context.active_object
...
for cord in pos:
    bpy.ops.mesh.primitive_cube_add(location=(cord[0] - center, cord[1] - center, 0.5), scale=(cord[3], cord[2], 2 * 0.5))
    ob = bpy.context.active_object
    mat = bpy.data.materials.get("Wall")
    if ob.data.materials:
        # assign to 1st material slot
